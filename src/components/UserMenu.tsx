...
export const UserMenu: React.FC<UserMenuProps> = ({
  ...
  useEffect(() => {
    ...
    if (mode === 'auto' && enabled && username) {
      const syncState = (window as any).__puretvLocalSettingsAutoPull as
        | { username: string; promise: Promise<boolean> }
        | undefined;
      if (!syncState || syncState.username !== username) {
        (window as any).__puretvLocalSettingsAutoPull = {
          username,
  ...
  const applyRemotePayload = (payload: LocalSettingsPayload | null) => {
    ...
    window.dispatchEvent(
      new CustomEvent('puretv_local_settings_applied', {
        detail: { payload },
      })
    );
  };
  ...
  useEffect(() => {
    ...
    window.addEventListener('puretv_local_settings_applied', handler);
    return () => {
      window.removeEventListener('puretv_local_settings_applied', handler);
    };
              ...
              {tvAccessTab === 'orion' && (
                <section className='rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-white/10 dark:bg-white/4'>
                  ...
                  <p className='mt-5 text-sm leading-6 text-slate-600 dark:text-slate-400'>
                    可直接作为 PureTV 电视端使用，适合安装到 Android TV /
                    电视盒子。
                  </p>
                  <div className='mt-5'>
                ...
                <div className='flex items-start gap-4'>
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-sm text-gray-600 dark:text-gray-400 mb-3'>
                      上游 Windows 客户端，支持私人影库 MKV；使用前请确认已适配 PureTV 的新协议。
                    </p>
                    <a
              ...
              <div className='bg-gray-50 dark:bg-gray-800 rounded-lg p-5 border border-gray-200 dark:border-gray-700'>
                <div className='flex items-start gap-4'>
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-sm text-gray-600 dark:text-gray-400 mb-3'>
                      上游增强插件，提供私人影库超分跨域能力；使用前请确认已适配 PureTV 的新站点标记。
                    </p>
                    <a
