...
const DEVICE_ID_KEY = 'puretv_tv_remote_device_id';
const LOCAL_REMOTE_URL_KEY = 'puretv_local_remote_url';
...
export default function TVRemoteReceiver() {
  useEffect(() => {
    const syncLocalRemoteUrl = () => {
      ...
      try {
        ...
        if (url.startsWith('http://') || url.startsWith('https://')) {
          ...
          window.__PURETV_LOCAL_REMOTE_URL = url;
          window.dispatchEvent(new CustomEvent('puretv:local-remote-info', {
            detail: { url },
          }));
        }
    ...
    window.addEventListener('puretv:local-remote-key', onLocalRemoteKey);
    window.addEventListener('puretv:local-remote-text', onLocalRemoteText);

    const auth = getAuthInfoFromBrowserCookie();
    if (!auth?.username) {
      return () => {
        window.removeEventListener('hashchange', syncLocalRemoteUrl);
        window.removeEventListener('puretv:local-remote-key', onLocalRemoteKey);
        window.removeEventListener('puretv:local-remote-text', onLocalRemoteText);
      };
    }
