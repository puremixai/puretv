...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 'puretv_play_records';
...
const FAVORITES_KEY = 'puretv_favorites';
const MANGA_SHELF_KEY = 'puretv_manga_shelf';
const MANGA_HISTORY_KEY = 'puretv_manga_history';
const DEFAULT_MAX_MANGA_HISTORY_RECORDS = 100;
const DEFAULT_MAX_MANGA_HISTORY_THRESHOLD =
  DEFAULT_MAX_MANGA_HISTORY_RECORDS + 10;
const SEARCH_HISTORY_KEY = 'puretv_search_history';
const MUSIC_PLAY_RECORDS_KEY = 'puretv_music_play_records';

// 缓存相关常量
const CACHE_PREFIX = 'puretv_cache_';
...
class HybridCacheManager {
  ...
  private clearAllCache(): void {
    ...
    keys.forEach((key) => {
      if (key.startsWith('puretv_cache_')) {
...
export async function getSkipConfig(
  ...
  // localStorage 模式
  try {
    const raw = localStorage.getItem('puretv_skip_configs');
    if (!raw) return null;
...
export async function saveSkipConfig(
  ...
  try {
    ...
    configs[key] = config;
    localStorage.setItem('puretv_skip_configs', JSON.stringify(configs));
    window.dispatchEvent(
      new CustomEvent('skipConfigsUpdated', {
        detail: configs,
      })
...
export async function getDanmakuFilterConfig(): Promise<DanmakuFilterConfig | null> {
  ...
  // localStorage 模式
  try {
    const raw = localStorage.getItem('puretv_danmaku_filter_config');
    if (!raw) return null;
...
export async function getEpisodeFilterConfig(): Promise<EpisodeFilterConfig | null> {
  ...
  try {
    const raw = localStorage.getItem('puretv_episode_filter_config');
    if (!raw) return null;
