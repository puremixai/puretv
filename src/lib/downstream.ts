...
interface ApiSearchItem {
  puretv_episodes?: string[];
  puretv_episode_titles?: string[];
  vod_id: string;
  vod_name: string;
  vod_pic: string;
  vod_remarks?: string;
...
async function searchWithCache(
  ...
  try {
    ...
    // 处理结果数据
    const nativeCms = response.headers.get('x-puretv-cms-native') === '1';
    const allResults = data.list.map((item: ApiSearchItem) => {
      let episodes: string[] = nativeCms ? item.puretv_episodes || [] : [];
      let titles: string[] = nativeCms ? item.puretv_episode_titles || [] : [];
