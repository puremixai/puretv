...
export function saveDanmakuSourceIndex(title: string, selectedIndex: number): void {
  ...
  try {
    ...
    logger.debug(`[弹幕记忆] 保存弹幕源下标: ${title} -> ${selectedIndex}`);
  } catch (error) {
  ...
  try {
    ...
    if (value !== null) {
      ...
      if (!isNaN(index) && index >= 0) {
        logger.debug(`[弹幕记忆] 读取弹幕源下标: ${title} -> ${index}`);
        return index;
      }
    }
  } catch (error) {
...
export function saveManualDanmakuSelection(
  ...
  try {
    ...
    logger.debug(`[弹幕记忆] 保存手动选择: ${title} 第${episodeIndex}集 -> ${episodeId}`);
  } catch (error) {
...
export function getManualDanmakuSelection(
  ...
  try {
    ...
    if (value !== null) {
      ...
      if (!isNaN(episodeId)) {
        logger.debug(`[弹幕记忆] 读取手动选择: ${title} 第${episodeIndex}集 -> ${episodeId}`);
        return episodeId;
      }
    }
  } catch (error) {
...
export function saveDanmakuSearchKeyword(title: string, keyword: string): void {
  ...
  try {
    ...
    logger.debug(`[弹幕记忆] 保存搜索关键词: ${title} -> ${keyword}`);
  } catch (error) {
...
export function getDanmakuSearchKeyword(title: string): string | null {
  ...
  try {
    ...
    if (keyword) {
      logger.debug(`[弹幕记忆] 读取搜索关键词: ${title} -> ${keyword}`);
      return keyword;
    }
  } catch (error) {
...
export function saveDanmakuAnimeId(title: string, animeId: number): void {
  ...
  try {
    ...
    logger.debug(`[弹幕记忆] 保存动漫ID: ${title} -> ${animeId}`);
  } catch (error) {
...
export function getDanmakuAnimeId(title: string): number | null {
  ...
  try {
    ...
    if (value !== null) {
      ...
      if (!isNaN(animeId)) {
        logger.debug(`[弹幕记忆] 读取动漫ID: ${title} -> ${animeId}`);
        return animeId;
      }
    }
  } catch (error) {
