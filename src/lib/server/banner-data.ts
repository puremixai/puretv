...
import type {
  ...
  BannerItem,
  BannerSource,
} from '@/lib/home/banner-types';
import { logger } from '@/lib/logger';
import { getTMDBTrendingContent, getTMDBVideos } from '@/lib/tmdb.client';
import { getDefaultTmdbImageBaseUrl } from '@/lib/tmdb-image-base';
import type { DoubanImageProxyType } from '@/lib/utils';

const CACHE_DURATION = 3 * 60 * 60 * 1000;
const INITIAL_TIMEOUT = 1500;
const MAX_CACHE_ENTRIES = 12;
const cache = new Map<string, { data: BannerData; timestamp: number }>();
const inFlight = new Map<string, Promise<BannerData>>();

type BannerErrorBody = {
  code: number;
  message?: string;
  list?: BannerItem[];
  source?: BannerSource;
...
/** Provider failures historically use HTTP 200; configuration failures use 400. */
export class BannerDataError extends Error {
  constructor(readonly body: BannerErrorBody, readonly status = 200) {
    super(body.message || '获取热门内容失败');
    this.name = 'BannerDataError';
  }
}

function normalizeError(error: unknown): BannerDataError {
  if (error instanceof BannerDataError) return error;
  logger.error('获取热门内容失败:', error);
  return new BannerDataError({ code: 500, message: '获取热门内容失败' }, 500);
}

/** Select only public display fields before crossing the server/client boundary. */
function publicItem(item: BannerItem): BannerItem {
  return {
    id: item.id,
    title: item.title,
...
function getConfiguredBannerData(config: AdminConfig): Promise<BannerData> {
  const site = config.SiteConfig;
  const configuredSource = site?.BannerDataSource || 'Douban';
  const source: BannerSource =
    configuredSource === 'Douban' || configuredSource === 'TX'
      ? configuredSource
      : 'TMDB';
  const apiKey = site?.TMDBApiKey;
  const proxy = site?.TMDBProxy;
  const reverseProxy = site?.TMDBReverseProxy;
  if (source === 'TMDB' && !apiKey) {
    return Promise.reject(
      new BannerDataError({ code: 400, message: 'TMDB API Key 未配置' }, 400)
    );
  }

  // Keys remain server-only and distinguish config changes, including in-flight work.
  const key = JSON.stringify([
    ...
    proxy,
    reverseProxy,
    site?.TMDBImageBaseUrl,
    site?.DoubanProxyType,
    site?.DoubanProxy,
  ]);
  ...
  const operation = (async (): Promise<BannerData> => {
    try {
      let result: { code: number; list: BannerItem[] };
      if (source === 'Douban') result = await getDoubanBannerContent();
      else if (source === 'TX') result = await getTXBannerContent();
      else {
        // The key check above runs before the cache lookup, including key removal.
        const tmdbResult = await getTMDBTrendingContent(
          apiKey || '',
          proxy,
          reverseProxy
        );
        result = tmdbResult;
        if (tmdbResult.code === 200) {
          result = {
            code: 200,
            list: await Promise.all(
              tmdbResult.list.map(async (item) => ({
                ...item,
                video_key: await getTMDBVideos(
                  apiKey || '',
                  item.media_type,
                  item.id,
                  proxy,
      ...
      if (result.code !== 200) {
        throw new BannerDataError({
          code: result.code,
          list: result.list.map(publicItem),
          source,
        });
      }
...
/** Shared by the public API and server-rendered homepage. */
export async function getBannerData(): Promise<BannerData> {
  try {
    return await getConfiguredBannerData(await getConfig());
  } catch (error) {
    throw normalizeError(error);
  }
}

type InitialBannerSeed = {
  data: BannerData;
  artwork: InitialBannerArtwork;
};

/** A slow provider may warm the shared cache without delaying the homepage. */
export function getInitialBannerSeed({
  timeoutMs = INITIAL_TIMEOUT,
}: { timeoutMs?: number } = {}): Promise<InitialBannerSeed | null> {
  ...
  const request = (async () => {
    try {
      ...
      if (
        site &&
        'HomeBannerEnabled' in site &&
        site.HomeBannerEnabled === false
      )
        return null;
      // Capture only public image settings before awaiting provider work. No secret
      // configuration is serialized or read again just to construct SSR artwork.
      const imageConfig = {
        tmdbImageBaseUrl:
          site?.TMDBImageBaseUrl || getDefaultTmdbImageBaseUrl(),
        doubanImageProxyType: (site?.DoubanImageProxyType ||
          'cmliussss-cdn-tencent') as DoubanImageProxyType,
        doubanImageProxyUrl: site?.DoubanImageProxy || '',
      };
      const data = await getConfiguredBannerData(config);
      return { data, artwork: createInitialBannerArtwork(data, imageConfig) };
    } catch (error) {
  ...
  return new Promise((resolve) => {
    let settled = false;
    const complete = (data: InitialBannerSeed | null) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      resolve(data);
    };
    const timer = setTimeout(() => complete(null), budget);
    // Both paths are handled even after a timeout; no detached rejection is created.
    void request.then(complete, () => complete(null));
  });
}

/** Data-only compatibility for callers that do not need the SSR artwork snapshot. */
export async function getInitialBannerData(
  options: { timeoutMs?: number } = {}
): Promise<BannerData | null> {
  return (await getInitialBannerSeed(options))?.data ?? null;
}

/**
 * 获取TX轮播图内容
 */
async function getTXBannerContent(): Promise<{
  code: number;
  list: BannerItem[];
}> {
  try {
 ...
 */
function parseTXBannerData(data: any): BannerItem[] {
  try {
