...
type PureTVLocalRemoteBridge = {
  getRemoteUrl?: () => string;
};

declare global {
  interface Window {
    PureTVLocalRemote?: PureTVLocalRemoteBridge;
    __PURETV_LOCAL_REMOTE_URL?: string;
  }
}
...
export default function TVMePage() {
  ...
  useEffect(() => {
    const readLocalRemoteUrl = () => {
      const bridgeUrl = window.PureTVLocalRemote?.getRemoteUrl?.() || '';
      setLocalRemoteUrl(
        bridgeUrl ||
        window.__PURETV_LOCAL_REMOTE_URL ||
    ...
    readLocalRemoteUrl();
    window.addEventListener('puretv:local-remote-info', onLocalRemoteInfo);
    const timer = window.setInterval(readLocalRemoteUrl, 1500);

    return () => {
      window.removeEventListener('puretv:local-remote-info', onLocalRemoteInfo);
