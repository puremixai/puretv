...
func TestResponseWriteDeadlineReleasesAdmission(t *testing.T) {
	...
	payload, _ := json.Marshal(map[string]any{"url": upstream.URL, "operation": "search", "query": "x"})
	makeRequest := func(invalid bool) *http.Request {
		body := payload
		if invalid {
			body = []byte(`{`)
		}
		return httptest.NewRequest("POST", "/v1/cms", bytes.NewReader(body))
	}
	for _, invalid := range []bool{false, true} {
		done := make(chan *nonreadingWriter, 16)
