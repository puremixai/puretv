...
func TestResponseWriteDeadlineReleasesAdmission(t *testing.T) {
	...
	payload, _ := json.Marshal(map[string]any{"url": upstream.URL, "username": "u", "password": "p", "path": "/api/me", "method": "GET"})
	makeRequest := func(invalid bool) *http.Request {
		body := payload
		if invalid {
			body = []byte(`{`)
		}
		return httptest.NewRequest("POST", "/v1/openlist/operations", bytes.NewReader(body))
	}
	for _, invalid := range []bool{false, true} {
		done := make(chan *nonreadingWriter, 2)
